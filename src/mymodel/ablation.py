...
class WoAdv(nn.Module):
    def __init__(self, num_labels, hidden=128, backbone='bert_base', geo_encoder='gcn_emb', n_layers_freeze=0, fusion='cls'):
        super(WoAdv, self).__init__()
...
class WOGeoenc(nn.Module):
    def __init__(self, num_labels, num_topics, hidden=128, backbone='bert_base', fusion='cls'):
        super(WOGeoenc, self).__init__()
        ...
        self.bert_hidden = config.hidden_size
        self.fusion = fusion
        self.final_hidden = self.bert_hidden*2
    ...
    def forward(self, x_input, input_key, opt):
        ...
        # step 3: features fusion module for prediction
        policy_fusion = torch.cat([policy_specific, policy_transfer], dim=-1)
...
class WGeoemb(nn.Module):
    def __init__(self, num_labels, num_topics, hidden=128, backbone='bert_base', geo_encoder='emb', n_layers_freeze=0, fusion='cls'):
        super(WGeoemb, self).__init__()
